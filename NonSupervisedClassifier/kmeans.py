...
# CLUSTERING
#################################
kmeans = KMeans(n_clusters=25, n_init=10) # 25 combinaisons entre Walc et Dalc
kmeans.fit(data)
...
# Plot separations
aa, bb, cc, dd, ee, ff, gg, hh, jj, kk, ll, mm, nn, oo, pp, qq, rr, ss, tt, uu, vv, ww, xx, yy, zz, aaa, bbb, ccc, ddd, eee = np.meshgrid(np.arange(data[:, 0].min() - 1, data[:, 0].max() + 1, .02), np.arange(data[:, 1].min() - 1, data[:, 1].max() + 1, .02),np.arange(data[:, 2].min() - 1, data[:, 2].max() + 1, .02))
